...
def ExtendPattern (parameters, pattern):
    ...
    while extendedInstances:
        ...
        newInstanceGraph = GraphS.CreateGraphFromInstance(newInstance)
        ...
        for extendedInstance in extendedInstances:
            extendedInstanceGraph = GraphS.CreateGraphFromInstance(extendedInstance)
            if parameters.temporal:
                extendedInstanceGraph.TemporalOrder()
            if GraphS.GraphMatch(newInstanceGraph,extendedInstanceGraph) and (not InstancesOverlap(parameters.overlap, matchingInstances, extendedInstance)):
...
def PatternListInsert(newPattern, patternList, maxLength, valueBased):
    ...
    for pattern in patternList:
        if (GraphS.GraphMatch(pattern.definition,newPattern.definition)):
